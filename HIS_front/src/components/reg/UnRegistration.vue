<template>
  <el-row>
    <el-col :span="24">
      ...
      <CommonTable :data_list="tableData" :table-headers="tableHeaders" :name="tableName" >表格</CommonTable>
    </el-col>
...
export default {
  ...
  data () {
    return {
      tableHeaders: ['病历号', '姓名', '身份证号','挂号日期', '午别', '看诊科室','看诊状态', '操作'],
      tableName: '挂号信息表',
      tableData: [],
      form: {
        caseNo: '',
  ...
  methods: {
    ...
    projection (registration) {
      ...
      let projection = []
      projection.push(registration.caseNo, registration.patientName, registration.personalId, registration.regTime, registration.noon, registration.deptId, registration.inspectionStatus)
      return projection
    },
    onSubmit () {
      console.log('submit!')
      this.$api.getAllPatientInfo(this.form.caseNo).then(successResponse => {
