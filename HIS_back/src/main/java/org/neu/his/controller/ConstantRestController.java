...
import org.neu.his.dao.ConstantMapper;
...
public class ConstantRestController{
    ...
    public CommonResponse getList(
        ...
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(Constant.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(Constant.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public Constant insertOne(@RequestBody Constant record) {
    ...
    @Transactional
    public List<Constant> insertBatch(@RequestBody List<Constant> records) {
        for (Constant record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody Constant record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<Constant> records) {
        int count = 0;
        for (Constant record : records) {
