...
import org.neu.his.dao.PaymentDtlMapper;
import org.neu.his.entity.PaymentDtl;
import org.neu.his.entity.PaymentDtlExample;
...
@Service
@RestController
@RequestMapping(path = "api/paymentDtl")
public class PaymentDtlRestController{
    @Resource
    PaymentDtlMapper mapper;
    ...
    public CommonResponse getList(
        ...
        PaymentDtlExample example = new PaymentDtlExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(PaymentDtl.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(PaymentDtl.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public PaymentDtl insertOne(@RequestBody PaymentDtl record) {
    ...
    @Transactional
    public List<PaymentDtl> insertBatch(@RequestBody List<PaymentDtl> records) {
        for (PaymentDtl record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody PaymentDtl record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<PaymentDtl> records) {
        int count = 0;
        for (PaymentDtl record : records) {
