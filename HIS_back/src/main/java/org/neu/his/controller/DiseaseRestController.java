...
import org.neu.his.dao.DiseaseMapper;
import org.neu.his.entity.Disease;
import org.neu.his.entity.DiseaseExample;
...
public class DiseaseRestController{
    ...
    public CommonResponse getList(
        ...
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(Disease.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(Disease.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public Disease insertOne(@RequestBody Disease record) {
    ...
    @Transactional
    public List<Disease> insertBatch(@RequestBody List<Disease> records) {
    ...
    @Transactional
    public int updateOne(@RequestBody Disease record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<Disease> records) {
        int count = 0;
        for (Disease record : records) {
