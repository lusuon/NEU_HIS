...
import org.neu.his.dao.TemplateDtlNonDrugMapper;
...
@Service
@RestController
@RequestMapping(path = "api/templateDtlNonDrug")
public class TemplateDtlNonDrugRestController{
    @Resource
    TemplateDtlNonDrugMapper mapper;
    ...
    public CommonResponse getList(
        ...
        TemplateDtlNonDrugExample example = new TemplateDtlNonDrugExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(TemplateDtlNonDrug.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(TemplateDtlNonDrug.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public TemplateDtlNonDrug insertOne(@RequestBody TemplateDtlNonDrug record) {
    ...
    @Transactional
    public List<TemplateDtlNonDrug> insertBatch(@RequestBody List<TemplateDtlNonDrug> records) {
        for (TemplateDtlNonDrug record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody TemplateDtlNonDrug record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<TemplateDtlNonDrug> records) {
        int count = 0;
        for (TemplateDtlNonDrug record : records) {
