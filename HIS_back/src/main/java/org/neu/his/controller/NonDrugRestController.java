...
import org.neu.his.dao.NonDrugMapper;
import org.neu.his.entity.NonDrug;
import org.neu.his.entity.NonDrugExample;
...
@Service
@RestController
@RequestMapping(path = "api/nonDrug")
public class NonDrugRestController{
    @Resource
    NonDrugMapper mapper;
    ...
    public CommonResponse getList(
        ...
        NonDrugExample example = new NonDrugExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(NonDrug.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(NonDrug.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public NonDrug insertOne(@RequestBody NonDrug record) {
    ...
    @Transactional
    public List<NonDrug> insertBatch(@RequestBody List<NonDrug> records) {
        for (NonDrug record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody NonDrug record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<NonDrug> records) {
        int count = 0;
        for (NonDrug record : records) {
