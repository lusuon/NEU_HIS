...
import org.neu.his.dao.CategoryDiseaseMapper;
import org.neu.his.entity.CategoryDisease;
import org.neu.his.entity.CategoryDiseaseExample;
...
@Service
@RestController
@RequestMapping(path = "api/categoryDisease")
public class CategoryDiseaseRestController{
    @Resource
    CategoryDiseaseMapper mapper;
    ...
    public CommonResponse getList(
        ...
        CategoryDiseaseExample example = new CategoryDiseaseExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(CategoryDisease.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(CategoryDisease.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public CategoryDisease insertOne(@RequestBody CategoryDisease record) {
    ...
    @Transactional
    public List<CategoryDisease> insertBatch(@RequestBody List<CategoryDisease> records) {
        for (CategoryDisease record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody CategoryDisease record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<CategoryDisease> records) {
        int count = 0;
        for (CategoryDisease record : records) {
