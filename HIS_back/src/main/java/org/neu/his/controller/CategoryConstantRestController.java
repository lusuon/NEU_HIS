...
import org.neu.his.dao.CategoryConstantMapper;
...
@Service
@RestController
@RequestMapping(path = "api/categoryConstant")
public class CategoryConstantRestController{
    @Resource
    CategoryConstantMapper mapper;
    ...
    public CommonResponse getList(
        ...
        CategoryConstantExample example = new CategoryConstantExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(CategoryConstant.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(CategoryConstant.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public CategoryConstant insertOne(@RequestBody CategoryConstant record) {
    ...
    @Transactional
    public List<CategoryConstant> insertBatch(@RequestBody List<CategoryConstant> records) {
        for (CategoryConstant record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody CategoryConstant record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<CategoryConstant> records) {
        int count = 0;
        for (CategoryConstant record : records) {
