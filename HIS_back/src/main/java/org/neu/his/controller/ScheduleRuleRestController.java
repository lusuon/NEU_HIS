...
import org.neu.his.dao.ScheduleRuleMapper;
import org.neu.his.entity.ScheduleRule;
import org.neu.his.entity.ScheduleRuleExample;
...
@Service
@RestController
@RequestMapping(path = "api/scheduleRule")
public class ScheduleRuleRestController{
    @Resource
    ScheduleRuleMapper mapper;
    ...
    public CommonResponse getList(
        ...
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(ScheduleRule.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(ScheduleRule.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public ScheduleRule insertOne(@RequestBody ScheduleRule record) {
    ...
    @Transactional
    public List<ScheduleRule> insertBatch(@RequestBody List<ScheduleRule> records) {
    ...
    @Transactional
    public int updateOne(@RequestBody ScheduleRule record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<ScheduleRule> records) {
        int count = 0;
        for (ScheduleRule record : records) {
