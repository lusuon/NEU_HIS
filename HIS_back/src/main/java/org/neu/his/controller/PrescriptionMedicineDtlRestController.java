...
import org.neu.his.dao.PrescriptionMedicineDtlMapper;
...
@Service
@RestController
@RequestMapping(path = "api/prescriptionMedicineDtl")
public class PrescriptionMedicineDtlRestController{
    @Resource
    PrescriptionMedicineDtlMapper mapper;
    ...
    public CommonResponse getList(
        ...
        PrescriptionMedicineDtlExample example = new PrescriptionMedicineDtlExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(PrescriptionMedicineDtl.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(PrescriptionMedicineDtl.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public PrescriptionMedicineDtl insertOne(@RequestBody PrescriptionMedicineDtl record) {
    ...
    @Transactional
    public List<PrescriptionMedicineDtl> insertBatch(@RequestBody List<PrescriptionMedicineDtl> records) {
        for (PrescriptionMedicineDtl record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody PrescriptionMedicineDtl record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<PrescriptionMedicineDtl> records) {
        int count = 0;
        for (PrescriptionMedicineDtl record : records) {
