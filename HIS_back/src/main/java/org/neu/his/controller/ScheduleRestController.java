...
import org.neu.his.dao.ScheduleMapper;
import org.neu.his.entity.Schedule;
import org.neu.his.entity.ScheduleExample;
...
public class ScheduleRestController{
    ...
    public CommonResponse getList(
        ...
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(Schedule.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(Schedule.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public Schedule insertOne(@RequestBody Schedule record) {
    ...
    @Transactional
    public List<Schedule> insertBatch(@RequestBody List<Schedule> records) {
    ...
    @Transactional
    public int updateOne(@RequestBody Schedule record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<Schedule> records) {
        int count = 0;
        for (Schedule record : records) {
