...
import org.neu.his.dao.AllUserMapper;
import org.neu.his.entity.AllUser;
import org.neu.his.entity.AllUserExample;
...
@Service
@RestController
@RequestMapping(path = "api/allUser")
public class AllUserRestController{
    @Resource
    AllUserMapper mapper;
    ...
    public CommonResponse getList(
        ...
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(AllUser.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(AllUser.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public AllUser insertOne(@RequestBody AllUser record) {
    ...
    @Transactional
    public List<AllUser> insertBatch(@RequestBody List<AllUser> records) {
        for (AllUser record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody AllUser record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<AllUser> records) {
        int count = 0;
        for (AllUser record : records) {
