...
import org.neu.his.dao.DrugMapper;
import org.neu.his.entity.Drug;
import org.neu.his.entity.DrugExample;
...
public class DrugRestController{
    ...
    public CommonResponse getList(
        ...
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(Drug.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(Drug.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public Drug insertOne(@RequestBody Drug record) {
    ...
    @Transactional
    public List<Drug> insertBatch(@RequestBody List<Drug> records) {
        for (Drug record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody Drug record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<Drug> records) {
        int count = 0;
        for (Drug record : records) {
