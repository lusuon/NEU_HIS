...
import org.neu.his.dao.TemplateNonDrugMapper;
...
@Service
@RestController
@RequestMapping(path = "api/templateNonDrug")
public class TemplateNonDrugRestController{
    @Resource
    TemplateNonDrugMapper mapper;
    ...
    public CommonResponse getList(
        ...
        TemplateNonDrugExample example = new TemplateNonDrugExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(TemplateNonDrug.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(TemplateNonDrug.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public TemplateNonDrug insertOne(@RequestBody TemplateNonDrug record) {
    ...
    @Transactional
    public List<TemplateNonDrug> insertBatch(@RequestBody List<TemplateNonDrug> records) {
        for (TemplateNonDrug record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody TemplateNonDrug record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<TemplateNonDrug> records) {
        int count = 0;
        for (TemplateNonDrug record : records) {
