...
import org.neu.his.dao.NonDrugApplicationMapper;
...
@Service
@RestController
@RequestMapping(path = "api/nonDrugApplication")
public class NonDrugApplicationRestController{
    @Resource
    NonDrugApplicationMapper mapper;
    ...
    public CommonResponse getList(
        ...
        NonDrugApplicationExample example = new NonDrugApplicationExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(NonDrugApplication.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(NonDrugApplication.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public NonDrugApplication insertOne(@RequestBody NonDrugApplication record) {
    ...
    @Transactional
    public List<NonDrugApplication> insertBatch(@RequestBody List<NonDrugApplication> records) {
        for (NonDrugApplication record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody NonDrugApplication record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<NonDrugApplication> records) {
        int count = 0;
        for (NonDrugApplication record : records) {
