...
import org.neu.his.dao.TemplateDtlHerbMapper;
...
@Service
@RestController
@RequestMapping(path = "api/templateDtlHerb")
public class TemplateDtlHerbRestController{
    @Resource
    TemplateDtlHerbMapper mapper;
    ...
    public CommonResponse getList(
        ...
        TemplateDtlHerbExample example = new TemplateDtlHerbExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(TemplateDtlHerb.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(TemplateDtlHerb.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public TemplateDtlHerb insertOne(@RequestBody TemplateDtlHerb record) {
    ...
    @Transactional
    public List<TemplateDtlHerb> insertBatch(@RequestBody List<TemplateDtlHerb> records) {
        for (TemplateDtlHerb record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody TemplateDtlHerb record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<TemplateDtlHerb> records) {
        int count = 0;
        for (TemplateDtlHerb record : records) {
