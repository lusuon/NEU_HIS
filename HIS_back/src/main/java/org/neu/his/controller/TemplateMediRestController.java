...
import org.neu.his.dao.TemplateMediMapper;
import org.neu.his.entity.TemplateMedi;
import org.neu.his.entity.TemplateMediExample;
...
@Service
@RestController
@RequestMapping(path = "api/templateMedi")
public class TemplateMediRestController{
    @Resource
    TemplateMediMapper mapper;
    ...
    public CommonResponse getList(
        ...
        TemplateMediExample example = new TemplateMediExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(TemplateMedi.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(TemplateMedi.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public TemplateMedi insertOne(@RequestBody TemplateMedi record) {
    ...
    @Transactional
    public List<TemplateMedi> insertBatch(@RequestBody List<TemplateMedi> records) {
        for (TemplateMedi record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody TemplateMedi record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<TemplateMedi> records) {
        int count = 0;
        for (TemplateMedi record : records) {
