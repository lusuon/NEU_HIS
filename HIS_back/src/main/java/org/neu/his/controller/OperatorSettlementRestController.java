...
import org.neu.his.dao.OperatorSettlementMapper;
...
@Service
@RestController
@RequestMapping(path = "api/operatorSettlement")
public class OperatorSettlementRestController{
    @Resource
    OperatorSettlementMapper mapper;
    ...
    public CommonResponse getList(
        ...
        OperatorSettlementExample example = new OperatorSettlementExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(OperatorSettlement.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(OperatorSettlement.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public OperatorSettlement insertOne(@RequestBody OperatorSettlement record) {
    ...
    @Transactional
    public List<OperatorSettlement> insertBatch(@RequestBody List<OperatorSettlement> records) {
        for (OperatorSettlement record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody OperatorSettlement record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<OperatorSettlement> records) {
        int count = 0;
        for (OperatorSettlement record : records) {
