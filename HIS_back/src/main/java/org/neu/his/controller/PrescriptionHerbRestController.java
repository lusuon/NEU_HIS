...
import org.neu.his.dao.PrescriptionHerbMapper;
...
@Service
@RestController
@RequestMapping(path = "api/prescriptionHerb")
public class PrescriptionHerbRestController{
    @Resource
    PrescriptionHerbMapper mapper;
    ...
    public CommonResponse getList(
        ...
        PrescriptionHerbExample example = new PrescriptionHerbExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(PrescriptionHerb.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(PrescriptionHerb.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public PrescriptionHerb insertOne(@RequestBody PrescriptionHerb record) {
    ...
    @Transactional
    public List<PrescriptionHerb> insertBatch(@RequestBody List<PrescriptionHerb> records) {
        for (PrescriptionHerb record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody PrescriptionHerb record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<PrescriptionHerb> records) {
        int count = 0;
        for (PrescriptionHerb record : records) {
