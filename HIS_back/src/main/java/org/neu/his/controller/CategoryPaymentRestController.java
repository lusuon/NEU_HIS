...
import org.neu.his.dao.CategoryPaymentMapper;
...
@Service
@RestController
@RequestMapping(path = "api/categoryPayment")
public class CategoryPaymentRestController{
    @Resource
    CategoryPaymentMapper mapper;
    ...
    public CommonResponse getList(
        ...
        CategoryPaymentExample example = new CategoryPaymentExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(CategoryPayment.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(CategoryPayment.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public CategoryPayment insertOne(@RequestBody CategoryPayment record) {
    ...
    @Transactional
    public List<CategoryPayment> insertBatch(@RequestBody List<CategoryPayment> records) {
        for (CategoryPayment record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody CategoryPayment record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<CategoryPayment> records) {
        int count = 0;
        for (CategoryPayment record : records) {
