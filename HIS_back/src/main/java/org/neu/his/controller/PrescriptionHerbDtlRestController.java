...
import org.neu.his.dao.PrescriptionHerbDtlMapper;
import org.neu.his.entity.PrescriptionHerbDtl;
import org.neu.his.entity.PrescriptionHerbDtlExample;
...
@Service
@RestController
@RequestMapping(path = "api/prescriptionHerbDtl")
public class PrescriptionHerbDtlRestController{
    @Resource
    PrescriptionHerbDtlMapper mapper;
    ...
    public CommonResponse getList(
        ...
        PrescriptionHerbDtlExample example = new PrescriptionHerbDtlExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(PrescriptionHerbDtl.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(PrescriptionHerbDtl.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public PrescriptionHerbDtl insertOne(@RequestBody PrescriptionHerbDtl record) {
    ...
    @Transactional
    public List<PrescriptionHerbDtl> insertBatch(@RequestBody List<PrescriptionHerbDtl> records) {
        for (PrescriptionHerbDtl record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody PrescriptionHerbDtl record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<PrescriptionHerbDtl> records) {
        int count = 0;
        for (PrescriptionHerbDtl record : records) {
