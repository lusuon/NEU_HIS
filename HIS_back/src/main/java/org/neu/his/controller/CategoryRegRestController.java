...
import org.neu.his.dao.CategoryRegMapper;
import org.neu.his.entity.CategoryReg;
import org.neu.his.entity.CategoryRegExample;
...
@Service
@RestController
@RequestMapping(path = "api/categoryReg")
public class CategoryRegRestController{
    @Resource
    CategoryRegMapper mapper;
    ...
    public CommonResponse getList(
        ...
        CategoryRegExample example = new CategoryRegExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(CategoryReg.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(CategoryReg.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public CategoryReg insertOne(@RequestBody CategoryReg record) {
    ...
    @Transactional
    public List<CategoryReg> insertBatch(@RequestBody List<CategoryReg> records) {
        for (CategoryReg record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody CategoryReg record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<CategoryReg> records) {
        int count = 0;
        for (CategoryReg record : records) {
