...
import org.neu.his.dao.DeptMapper;
...
public class DeptRestController{
    ...
    public CommonResponse getList(
        ...
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(Dept.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(Dept.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public Dept insertOne(@RequestBody Dept record) {
    ...
    @Transactional
    public List<Dept> insertBatch(@RequestBody List<Dept> records) {
    ...
    @Transactional
    public int updateOne(@RequestBody Dept record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<Dept> records) {
        int count = 0;
        for (Dept record : records) {
