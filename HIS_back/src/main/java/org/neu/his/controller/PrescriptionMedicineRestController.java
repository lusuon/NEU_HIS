...
import org.neu.his.dao.PrescriptionMedicineMapper;
import org.neu.his.entity.PrescriptionMedicine;
import org.neu.his.entity.PrescriptionMedicineExample;
...
@Service
@RestController
@RequestMapping(path = "api/prescriptionMedicine")
public class PrescriptionMedicineRestController{
    @Resource
    PrescriptionMedicineMapper mapper;
    ...
    public CommonResponse getList(
        ...
        PrescriptionMedicineExample example = new PrescriptionMedicineExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(PrescriptionMedicine.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(PrescriptionMedicine.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public PrescriptionMedicine insertOne(@RequestBody PrescriptionMedicine record) {
    ...
    @Transactional
    public List<PrescriptionMedicine> insertBatch(@RequestBody List<PrescriptionMedicine> records) {
        for (PrescriptionMedicine record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody PrescriptionMedicine record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<PrescriptionMedicine> records) {
        int count = 0;
        for (PrescriptionMedicine record : records) {
