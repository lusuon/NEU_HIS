...
import org.neu.his.dao.TemplateHerbMapper;
import org.neu.his.entity.TemplateHerb;
import org.neu.his.entity.TemplateHerbExample;
...
@Service
@RestController
@RequestMapping(path = "api/templateHerb")
public class TemplateHerbRestController{
    @Resource
    TemplateHerbMapper mapper;
    ...
    public CommonResponse getList(
        ...
        TemplateHerbExample example = new TemplateHerbExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(TemplateHerb.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(TemplateHerb.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public TemplateHerb insertOne(@RequestBody TemplateHerb record) {
    ...
    @Transactional
    public List<TemplateHerb> insertBatch(@RequestBody List<TemplateHerb> records) {
        for (TemplateHerb record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody TemplateHerb record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<TemplateHerb> records) {
        int count = 0;
        for (TemplateHerb record : records) {
