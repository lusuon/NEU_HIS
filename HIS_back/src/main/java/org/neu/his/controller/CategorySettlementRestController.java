...
import org.neu.his.dao.CategorySettlementMapper;
import org.neu.his.entity.CategorySettlement;
import org.neu.his.entity.CategorySettlementExample;
...
@Service
@RestController
@RequestMapping(path = "api/categorySettlement")
public class CategorySettlementRestController{
    @Resource
    CategorySettlementMapper mapper;
    ...
    public CommonResponse getList(
        ...
        CategorySettlementExample example = new CategorySettlementExample();

        //排序
        if (sort != null && order != null)
            switch (order.toLowerCase()) {
                case "asc":
                    example.setOrderByClause(CategorySettlement.Column.valueOf(sort).asc());
                    break;
                case "desc":
                    example.setOrderByClause(CategorySettlement.Column.valueOf(sort).desc());
                    break;
                default:
    ...
    @Transactional
    public CategorySettlement insertOne(@RequestBody CategorySettlement record) {
    ...
    @Transactional
    public List<CategorySettlement> insertBatch(@RequestBody List<CategorySettlement> records) {
        for (CategorySettlement record : records) {
            mapper.insertSelective(record);
        }
        return records;
    }
    ...
    @Transactional
    public int updateOne(@RequestBody CategorySettlement record) {
    ...
    @Transactional
    public long updateBatch(@RequestBody List<CategorySettlement> records) {
        int count = 0;
        for (CategorySettlement record : records) {
